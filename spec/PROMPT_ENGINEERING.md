...
```json
{
  ...
  "extraction_data": {
    "reasoning_process": "先思考…再提取…：通读 Related Work / Method 填写 evolution_lineages；再通读实验与对比章节填写 comparisons。",
...
class EvolutionLineage(BaseModel):
    ancestor_method: str = Field(description="被继承、扩展或受其启发的祖先方法名称")
    evolution_reason: str = Field(description="具体的演进原因或继承特性")
...
class KnowledgeGraphNodes(BaseModel):
    evolution_lineages: List[EvolutionLineage] = Field(
        ...
        description="技术血脉传承（写入图谱 :EVOLVED_FROM）。"
    )
